...
- **AI 选型助手**：内置大模型对话（OpenAI 兼容协议，支持阿里云百炼/DeepSeek 等），描述需求特征即可推荐具体型号；**推荐结果自动到立创商城核验真伪**并附**品牌/封装/实时库存/单价/基础库标记**，核验通过的一键填入导出框。
- **合并为单一库文件**：批量导完自动合成一个 `MergedLib.SchLib` + `MergedLib.PcbLib`（KiCad：`MergedLib.kicad_sym` + `MergedLib.pretty/`），EDA 里一次导入全搞定；分元件产物照常保留。
- **导出前预览**：一键画出原理图符号和 PCB 封装（引脚名、焊盘布局所见即所得），确认没抓错料再导出；结果表双击可复看。
- **自动更新**：启动时后台检查 GitHub 新版本，有更新弹窗直达下载页（无网静默）。
